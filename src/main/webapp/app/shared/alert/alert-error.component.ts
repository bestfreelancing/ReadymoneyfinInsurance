...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: JhiAlertService, private eventManager: JhiEventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('readymoneyfin.error', (response: JhiEventWithContent<AlertError>) => {
      const errorResponse = response.content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('readymoneyfin.httpError', (response: JhiEventWithContent<HttpErrorResponse>) => {
      // eslint-disable-next-line no-console
      console.info('Error got', response);
      ...
      switch (httpErrorResponse.status) {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName = translateService.instant('readymoneyfin.' + fieldError.objectName + '.' + convertedField);
        ...
        default:
          // eslint-disable-next-line no-console
          console.info('Default error', httpErrorResponse.error);
          if (httpErrorResponse.error !== '' && httpErrorResponse.error.message) {
            this.addErrorAlert(httpErrorResponse.error.message, 'error.internalServerError');
          } else {
            this.addErrorAlert(httpErrorResponse.error, 'error.internalServerError');
          }
      }
    });
  }
